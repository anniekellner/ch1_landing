...
head(area_mn_50)
area_mn_50$id.datetime <- paste(area_mn_50$id, area_mn_50$datetime)

area_mn_50 <- area_mn_50 %>% pivot_wider(names_from = c(class), values_from = c(class, value)) # spread rows into columns by class

full <- left_join(ice.df, area_mn_50, by = "id.datetime")# join ice.df with lsm df's
...
# Problems - 20845.2015 - RESOLVED 4/2/2020
# Make sure all rows have value_0 and value_3 before subsetting by value_3

lsmtest <- subset(lsm, id=="pb_20845.2015") # lsm looks good 
lsm10test <- subset(lsm10, id=="pb_20845.2015") # Problem was within code at this step= had filtered by 3's before all dates were accounted for
areatest <- subset(area_mn_10, id=="pb_20845.2015")
fulltest <- subset(full, id=="pb_20845.2015")
