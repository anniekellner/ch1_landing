...
ss <- subset(bears, start.swim == 1) # 16 swims
...
avg <- bears %>% # Compute daily average
  group_by(id, ymd) %>%
  dplyr::summarise(
    first(animal), mean(SIC_30m_me), mean(SIC_30m_max), mean(SIC_30m_min), mean(SICsq), mean(dist2land), max(start.swim), max(Bonepile)
    ) %>%
  ungroup()
...
colnames(avg) <- c("id", "ymd", "animal", "SIC_mean", "SIC_max", "SIC_min", "SICsq", "Dist2land", "start.swim", "Bonepile")
...
SIC_max <- coxph(Surv(tstart, tstop, start.swim) ~ SIC_max, data = avg)

SIC_mean <- coxph(Surv(tstart, tstop, start.swim) ~ SIC_mean, data = avg)

SIC_sq <- coxph(Surv(tstart, tstop, start.swim) ~ SICsq, data = avg)

SIC_min <- coxph(Surv(tstart, tstop, start.swim) ~ SIC_min, data = avg)
...
#test <- subset(wind1, wind1$id %in% lb)
...
#ss.new <- new$start.swim
#animal <- ss$animal 

wind1 <- dplyr::select(wind1, timestamp, location.long, location.lat, ht.above.ellipsoid, id, NCEP.NARR.FLX.U.Wind.at.10.m.above.Ground, NCEP.NARR.FLX.V.Wind.at.10.m.above.Ground) # This csv has data for both ice and land bears
...
lb <- unique(bears$id)
wind <- subset(wind, id %in% lb)

windBears <- unique(wind$id)

setdiff(lb, windBears)


test <- left_join(bears, wind, by = c("datetime","id"))
...
wind <- wind[!is.na(wind$Wind_U),]
wind <- wind[!is.na(wind$Wind_V),]
