...
bears <- bears %>%
  ...
  rename(id = id.x) %>%
  dplyr::select(animal, year, start.swim, SIC_30m_me:SIC_30m_min, id, datetime, id.datetime, SICsq, pland, te, dist2land)
...
avg <- wind.join %>% # Compute daily average
  group_by(id, yday) %>%
  dplyr::summarise(
    first(animal), mean(SICsq), mean(SIC_30m_me), mean(SIC_30m_max), mean(SIC_30m_min), mean(dist2land), max(start.swim), mean(speed)) %>%
  ungroup()

colnames(avg) <- c("id", "ordinal_day", "animal", "SICsq", "SICmean", "SICmax", "SICmin", "MeanDist2land", "start.swim", "Windspeed") 
...
# Add rate of change and cumulative rate of change

avg$sd7 <- rollapplyr(avg$SICmean, 7, sd, fill = NA)

rollapplyr(cox_tdc$SIC_mean, 7, sd, fill = NA) #sd over 7 days
...
cox_cum_tdc <- tmerge(baseline, avg, id = id, SIC_mean = cumtdc(day, SICmean), SICmax = cumtdc(day, SICmax), SICmin = cumtdc(day, SICmin), SICsq = cumtdc(day, SICsq), sd7 = cumtdc(day, sd7), Distance_to_land = tdc(day, MeanDist2land), Windspeed = tdc(day, Windspeed))

cox_tdc <- tmerge(baseline, avg, id = id, SICmean = tdc(day, SICmean), SICmax = tdc(day, SICmax), SICmin = tdc(day, SICmin), SICsq = tdc(day, SICsq), sd7 = tdc(day, sd7), Distance_to_land = tdc(day, MeanDist2land), Windspeed = tdc(day, Windspeed))
...
```{r Sea Ice Concentration AIC tdc}

# CoxPH (baseline hazard not specified)

SIC_max <- coxph(Surv(tstart, tstop, migrate) ~ SICmax, cluster = id, data = cox_tdc)
#SIC_maxMax <- coxph(Surv(tstart, tstop, migrate) ~ SICmaxMax, , data = avg)
SIC_mean <- coxph(Surv(tstart, tstop, migrate) ~ SICmean, cluster = id, data = cox_tdc)
...
SIC_max <- coxph(Surv(tstart, tstop, migrate) ~ SICmax, cluster = id, data = cox_cum_tdc)
#SIC_maxMax <- coxph(Surv(tstart, tstop, migrate) ~ SICmaxMax, , data = avg)
SIC_mean <- coxph(Surv(tstart, tstop, migrate) ~ SIC_mean, cluster = id, data = cox_cum_tdc)

SIC_sq <- coxph(Surv(tstart, tstop, migrate) ~ SICsq, cluster = id, data = cox_cum_tdc)
...
```{r Add sd and compare to SIC}

# Added sd and cumulative sd to model list. SICsq (not cumulative) is VERY clearly the best model. However the direction of the effect overlaps 0, though leans negative (which makes sense) 

SIC_sq <- coxph(Surv(tstart, tstop, migrate) ~ SICsq, cluster = id, data = cox_tdc)
SIC_max <- coxph(Surv(tstart, tstop, migrate) ~ SICmax, cluster = id, data = cox_cum_tdc)
sd_cum <- coxph(Surv(tstart, tstop, migrate) ~ sd7, cluster = id, data = cox_cum_tdc)
sd <- coxph(Surv(tstart, tstop, migrate) ~ sd7, cluster = id, data = cox_tdc)


aicc <- AICc(SIC_max, SIC_sq, sd_cum, sd)
...
# Test which moving window yields the strongest result

# Look at the data: recordhow many days have high wind speeds before departure

which(cox_tdc$migrate == 1)

cox_tdc <- cox_tdc %>%
  mutate(Windspeed_3prior = lag(rollmean(SICsq, 3, fill = NA, align = "right"))) %>%
  mutate(Windspeed_3davg = rollmean(Windspeed, 3, fill = NA, align = "right")) %>%
  mutate(Windspeed_5davg = rollmean(Windspeed, 5, fill = NA, align = "right")) %>%
  mutate(Windspeed_7davg = rollmean(Windspeed, 7, fill = NA, align = "right")) 

prior3 <- coxph(Surv(tstart, tstop, migrate) ~ Windspeed_3prior, cluster= animal, data = cox_tdc)
avg3 <- coxph(Surv(tstart, tstop, migrate) ~ Windspeed_3davg, cluster= animal, data = cox_tdc)
avg5 <- coxph(Surv(tstart, tstop, migrate) ~ Windspeed_5davg, cluster= animal, data = cox_tdc)
avg7 <- coxph(Surv(tstart, tstop, migrate) ~ Windspeed_7davg, cluster= animal, data = cox_tdc)

aicc <- AICc(prior3, avg3, avg5, avg7)

create_AICc_table(aicc)

```

3-day moving window is the best.
