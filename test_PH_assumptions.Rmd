...
*Martingale residuals* assess nonlinearity
...
```{r}
ggcoxfunctional(Surv(tstart, tstop, migrate) ~ sqrt(SIC_mean), data = ph)


fcoxph(Surv(tstart, tstop, migrate) ~ SIC_mean + speed3_max_mean + dist_land + dist_pack + sd7 + ResidMass + dir, 
                      cluster = animal, data = ph)
```
