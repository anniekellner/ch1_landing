...
rl <- dir(path = "./SIC-TIFs/SIC_univ_Bremen/RCC", pattern='.tif', all.files=TRUE, recursive = TRUE, full.names=FALSE)

# Create directories for new polygons

for(i in 1:length(ded)){
dir.create(paste0('./SIC-TIFs/SIC_univ_Bremen/POLY/', ded[i]))
}
...
bat_pack_ice_poly <- function(rl){
  for (i in 1:length(rl)) {
    r <- raster(paste0("./SIC-TIFs/SIC_univ_Bremen/RCC/", rl[i])) #read in raster
    ...
    poly <- st_as_sf(poly) #convert to sf object
    ...
    st_write(poly, paste0('./SIC-TIFs/SIC_univ_Bremen/POLY/', rl[i]), driver='ESRI shapefile', delete_layer=TRUE)}}
