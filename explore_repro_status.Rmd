...
bears <- readRDS('./data/RData/ph_Apr26_2022.Rds')
repro <- readRDS('./data/derived-data/repro.Rds')

repro <- select(repro, id, repro)

# Add repro data to PH dataset

ph <- inner_join(bears, repro)
...
repro_model_factors <- coxph(Surv(tstart, tstop, migrate) ~ repro, cluster = id, data = ph)

summary(repro_model_factors)
...
```{r ph-offspring, echo=FALSE}

ph2 <- ph %>%
  mutate(repro = replace(repro, repro == "enter_den", 0)) %>%
  mutate(repro = replace(repro, repro == "coy", 1)) %>%
  mutate(repro = replace(repro, repro == "yearling", 1))

repro_model_dependent_offspring <- coxph(Surv(tstart, tstop, migrate) ~ repro, data = ph2, cluster = id)

summary(repro_model_dependent_offspring)

```

What if the classification is COY or NO COY?
...
ph3 <- ph %>%
  mutate(repro = replace(repro, repro == "enter_den", 0)) %>%
  mutate(repro = replace(repro, repro == "coy", 1)) %>%
  mutate(repro = replace(repro, repro == "yearling", 0))

repro_model_coy <- coxph(Surv(tstart, tstop, migrate) ~ repro, data = ph3, cluster = id)

summary(repro_model_coy)
